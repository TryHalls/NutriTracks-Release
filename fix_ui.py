...
with open('js/modules/ui.js', 'r') as f:
    content = f.read()
...
export function initScannerEvents() {
  ...
  if (btnFallbackManual) {
    btnFallbackManual.addEventListener('click', function() {
      closeScannerModal();
      openAddFood(null, ScannerState.selectedMeal);
    });
  }
...
open_scanner_mod = """export function openScannerModal() {
  ...
  if (greeting === 'Buenos días') ScannerState.selectedMeal = 'breakfast';
  else if (greeting === 'Buenas tardes') ScannerState.selectedMeal = 'lunch';
...
register_scanned = """export function _registerScannedProduct(food, qty, sourceOverride) {
...
"""
content = re.sub(r'export function _registerScannedProduct\(food, qty, sourceOverride\) \{[\s\S]*?(?=export function exportData)', register_scanned, content)
...
with open('js/modules/ui.js', 'w') as f:
    f.write(content)
